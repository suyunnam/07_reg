...
# Model fitting
mod3_quad <- lmer(yield_mgha ~ (1|rep) + sr_ksha + I(sr_ksha^2),
                  data = reg_dfw)
...
# Model fitting (nlme is needed for non-linear & mixed model)
mod4_linp <- nlme(yield_mgha ~ SSlinp(sr_ksha, a, b, xs),
                  data = reg_dfw,
                  random = list(rep = pdDiag(a + b + xs ~ 1)),
                  fixed = list(a ~ 1, b ~ 1, xs ~ 1),
                  start = c(a = 0, b = 0.3, xs = 100)   # Provide initial parameter guesses
                  )                                     # The model will iterate and find the better and accurate value
...
# AICc is more conservative than AIC. 
# dAIC will show the best model as 0, and others are compared to that one
...
predict(mod4_linp,
        ...
        level = 0) # do not specify block. we want to know general y value at specific x value

```
At the optimum seeding rate of **73.48** thousand seeds/ha, corn grain yield was **13.45** Mg/ha.
...
The goal of bootstrapping is estimating **confidence intervals**

Bootstrapping is best suited where the original data have been generated through **random sampling** from a population

Bootstrapping is also referred to as **sampling with replacement**. So for ex., if you initial sample has 10 observations, bootsrapping will take a total of 10 samples on each run, where:

- takes the first sample (say, row 3)
- takes the second sample (say, row 5)
- takes the third sample (say, row 1)
- takes the fourth sample (say, row 3 again)
- ...
- takes the 10th sample (say, row 4) (this is the end of the first sampling run)

And do this subsampling more than 1000 times, for research purposes

Notice how row 3 apeared twice in the first sampling run. This is what "with replacement" means: you don't exclude a given row from being sampled again just because it was already sampled.

## Marble bag and corn yield
In experimentation, the data we collect is just a sample from the larger population of potential data.

In our example of yield in this exercise, we collected **20 total observations**.

There observations are one realization of an infinite set of possible realizations (image, for ex., if you had sampled just to the side of the corn row, or maybe a different corn row, or maybe a slightly different site in the farm).

Normally, we make inference on our sample only.

With bootstrap, we can create many potential samples based on the single one we collected (the individual resamples on the far right of the figure).

In the example below
 - **Population** would be our infinite potential data from a population
 - **Sample** is what we collect from that (i.e., out yield data)
 - **Resample** are the different sampling runs with replacement (bootstrapping) from our original sample
 
Each sample has the **same number of observations** as the original sample, with replacement.

## Creating a bootstrapped confidence interval for the yield data
First, let's generate 10,000 resamples (with replacement) from the yield data we've been working with.
...
How many rows do we have above? why?
200,000 = 20 (sample number) X 10,000 (times of resampling)

Let's inspect the first bootstrapped sampled (i.e., replicate column):
...
What do you see? Any repeated values?

Now, let's calculate the mean for each bootstrapped sample.
...
10,000. as many times as we re-sampled.

Now, let's calculated the 2.5% and 98.7% quantiles of the mean distribution. There will be the limits of our confidence interval.
...
Finally, let's visualize the mean and CI.
```{r}
...
```
## Interpretation
Confidence interval is the **range of plausible values for our parameter of interest**

Confidence intervals are often misinterpreted as probability...

How do we interpret this?
A) There is a 95% probability the true mean corn yield is between 11.2 and 13 Mg/ha.

B) There is a 95% probability the mean corn yield is between 11.2 and 13 Mg/ha.

**C) We are 95% confident the true mean corn yield is between 11.2 and 13 Mg/ha.**

D) We are 95% confident the corn yield is between 11.2 and 13 Mg/ha
